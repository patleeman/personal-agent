...
function parseRemoteServerConfig(name, value, baseDir) {
  ...
  const oauthClientMetadata =
    parseJsonArg(typeof oauth?.clientMetadataPath === 'string' ? `@${oauth.clientMetadataPath}` : undefined, baseDir) ??
    (isRecord(oauth?.clientMetadata) ? oauth.clientMetadata : undefined);
  const explicitClientInfo =
    parseJsonArg(typeof oauth?.clientInfoPath === 'string' ? `@${oauth.clientInfoPath}` : undefined, baseDir) ??
  ...
  const ignoreTools = Array.isArray(value.ignoreTools) ? value.ignoreTools.filter((entry) => typeof entry === 'string') : undefined;
  return {
    name,
    transport: 'remote',
    args: [],
...
export async function connectMcpServerDirect(server, options = {}) {
  ...
  return {
    callTool: async (toolName, input, callTimeoutMs) => {
      ...
      return withTimeout(
        connection.client.callTool({ name: toolName, arguments: isRecord(input) ? input : {} }),
        t,
        `Calling ${server.name}/${toolName}`,
      );
    },
