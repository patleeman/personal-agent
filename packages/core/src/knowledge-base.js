...
import { DEFAULT_MACHINE_KNOWLEDGE_BASE_BRANCH, readMachineKnowledgeBaseBranch, readMachineKnowledgeBaseRepoUrl, writeMachineKnowledgeBase, } from './machine-config.js';
...
function computeRecoveryEntryId(relativePath, timestamp) {
...
function runGitBuffer(cwd, args, options = {}) {
...
function getRemoteRef(branch) {
    return `refs/remotes/origin/${branch}`;
}
function listRemoteSnapshot(cwd, branch) {
...
function listWorkingTreePaths(cwd) {
...
function computeWorkingBlobHash(cwd, relativePath) {
    return runGitText(cwd, ['hash-object', '--', relativePath]).trim();
}
function listWorkingSnapshot(cwd) {
    const snapshot = {};
...
function countWorkingTreeChanges(cwd) {
...
function readAheadBehindCounts(cwd, branch) {
    if (!headExists(cwd)) {
...
function readGitStatus(root, branch) {
...
function readSyncLockMetadata(filePath) {
...
function sanitizeRecoveryRelativePath(relativePath) {
...
function readStoredState(filePath) {
    ...
    try {
        ...
        const snapshot = {};
        for (const [path, entry] of Object.entries(parsed.snapshot)) {
...
function writeStoredState(filePath, state) {
    ensureParentDirectory(filePath);
    writeFileSync(filePath, `${JSON.stringify(state, null, 2)}\n`);
}
function clearStoredState(filePath) {
    rmSync(filePath, { force: true });
}
function readRecoveryIndex(filePath) {
    if (!existsSync(filePath)) {
        return [];
    }
    try {
        ...
        return Array.isArray(parsed.entries)
            ? parsed.entries.filter((entry) => typeof entry === 'string' && entry.trim().length > 0)
            : [];
    }
    catch {
        return [];
...
function appendRecoveryIndex(filePath, entryId) {
...
function removeEmptyParentDirectories(root, filePath) {
...
function deleteFileIfExists(filePath, root) {
    if (!existsSync(filePath)) {
        return;
    }
    unlinkSync(filePath);
...
function readRemoteFileBuffer(cwd, branch, relativePath) {
    const remoteRef = getRemoteRef(branch);
    if (!refExists(cwd, remoteRef)) {
        return null;
    }
...
function readLocalFileBuffer(root, relativePath) {
    const absolutePath = join(root, relativePath);
    return existsSync(absolutePath) ? readFileSync(absolutePath) : null;
}
function readRemotePathTimestampMs(cwd, branch, relativePath, existsInRemote) {
    const remoteRef = getRemoteRef(branch);
...
function readLocalPathTimestampMs(root, relativePath, existsInLocal) {
    if (!existsInLocal) {
        return Date.now();
    }
    try {
...
function hasRecentLocalChanges(root, baseSnapshot, workingSnapshot, nowMs) {
    const changedPaths = new Set([...Object.keys(baseSnapshot), ...Object.keys(workingSnapshot)]);
...
function tryRunGitCommand(cwd, args) {
    try {
        runGitText(cwd, args);
        return true;
    }
...
function runRepositoryMaintenance(root, task) {
    if (task === 'gc') {
...
function normalizeBranchTracking(root, branch, remoteName = 'origin') {
...
function maybeRunRepositoryMaintenance(root, storedState, timestamp) {
    ...
    const shouldRunFullMaintenance = lastFullMaintenanceAtMs !== null && nowMs - lastFullMaintenanceAtMs >= KNOWLEDGE_BASE_FULL_MAINTENANCE_INTERVAL_MS;
    ...
    const shouldRunAutoMaintenance = lastMaintenanceAtMs === null || nowMs - lastMaintenanceAtMs >= KNOWLEDGE_BASE_AUTO_MAINTENANCE_INTERVAL_MS;
...
function knowledgeBaseStateEquals(left, right) {
...
function setRuntimeState(runtimeState, input) {
    if (input.syncStatus) {
...
export class KnowledgeBaseManager {
    ...
    recoveryIndexPath;
    syncLockDir;
    syncLockMetadataPath;
    listeners = new Set();
    runtimeState;
    interval = null;
    syncInProgress = false;
    activeSyncLockTimestamp = null;
    syncBackupDir = null;
    constructor(options = {}) {
    ...
    readStoredStateForConfig(config) {
    ...
    checkoutRemoteBase(root, branch, remoteExists) {
        if (remoteExists) {
            const remoteRef = getRemoteRef(branch);
    ...
    notifyListeners(previousState, nextState) {
    ...
    writeRecoveryCopy(relativePath, content, timestamp) {
    ...
    backupWorkingTree(root, snapshot) {
    ...
    restoreWorkingTree(backupDir, root, snapshot) {
        for (const relativePath of Object.keys(snapshot)) {
    ...
    validateSyncResult(baseSnapshot, finalPaths) {
    ...
    stageAndCommitIfNeeded(root, branch, timestamp) {
    ...
    resolveChangedPaths(root, branch, baseSnapshot, workingSnapshot, remoteSnapshot) {
    ...
    syncNow(previousStateInput) {
        ...
        try {
            ...
            const managedExistingPaths = new Set([
            ...
            try {
                ...
                const finalPaths = new Set(Object.keys(remoteSnapshot));
    ...
    startSyncLoop(intervalMs = KNOWLEDGE_BASE_SYNC_INTERVAL_MS) {
...
export function readKnowledgeBaseState(options = {}) {
    return getKnowledgeBaseManager(options).readKnowledgeBaseState();
}
export function updateKnowledgeBase(input, options = {}) {
    return getKnowledgeBaseManager(options).updateKnowledgeBase(input);
}
export function syncKnowledgeBaseNow(options = {}) {
    return getKnowledgeBaseManager(options).syncNow();
}
export function subscribeKnowledgeBaseState(listener, options = {}) {
    return getKnowledgeBaseManager(options).subscribe(listener);
}
export function startKnowledgeBaseSyncLoop(options = {}) {
    getKnowledgeBaseManager(options).startSyncLoop(options.intervalMs);
}
export function stopKnowledgeBaseSyncLoop(options = {}) {
