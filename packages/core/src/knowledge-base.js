...
function readRecoveryIndex(filePath) {
  ...
  try {
    const parsed = JSON.parse(readFileSync(filePath, 'utf-8'));
    return Array.isArray(parsed.entries) ? parsed.entries.filter((entry) => typeof entry === 'string' && entry.trim().length > 0) : [];
  } catch {
    return [];
  }
}
...
export class KnowledgeBaseManager {
  ...
  syncNow(previousStateInput) {
    ...
    try {
      ...
      const managedExistingPaths = new Set([...Object.keys(workingSnapshot), ...Object.keys(remoteSnapshot), ...Object.keys(baseSnapshot)]);
