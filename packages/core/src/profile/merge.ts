...
function resolveBooleanValue(lower: unknown, higher: unknown, defaultValue: boolean, nullClearsValue: boolean): boolean {
  if (higher !== undefined) {
    if (higher === null) {
...
function resolveRequiredStringValue(lower: unknown, higher: unknown, defaultValue: string, nullClearsValue: boolean): string {
  if (higher !== undefined) {
    if (higher === null) {
      if (nullClearsValue) return defaultValue;
...
function resolveNullableStringValue(lower: unknown, higher: unknown, defaultValue: string | null, nullClearsValue: boolean): string | null {
  if (higher !== undefined) {
    if (higher === null) {
...
function mergeNotifications(
  ...
  return {
    email: resolveBooleanValue(lower?.email, higher?.email, DEFAULTS.notifications.email, nullClearsValue),
    push: resolveBooleanValue(lower?.push, higher?.push, DEFAULTS.notifications.push, nullClearsValue),
    digest: resolveDigestValue(lower?.digest, higher?.digest, DEFAULTS.notifications.digest, nullClearsValue),
  };
}
...
function mergePrivacy(
  ...
  return {
    analytics: resolveBooleanValue(lower?.analytics, higher?.analytics, DEFAULTS.privacy.analytics, nullClearsValue),
    shareUsage: resolveBooleanValue(lower?.shareUsage, higher?.shareUsage, DEFAULTS.privacy.shareUsage, nullClearsValue),
  };
}
...
function mergeModelPreferences(
  ...
  return {
    default: resolveRequiredStringValue(lower?.default, higher?.default, DEFAULTS.modelPreferences.default, nullClearsValue),
    coding: resolveNullableStringValue(lower?.coding, higher?.coding, DEFAULTS.modelPreferences.coding, nullClearsValue),
    analysis: resolveNullableStringValue(lower?.analysis, higher?.analysis, DEFAULTS.modelPreferences.analysis, nullClearsValue),
    creative: resolveNullableStringValue(lower?.creative, higher?.creative, DEFAULTS.modelPreferences.creative, nullClearsValue),
  };
}
...
function mergeToolPermissions(
  ...
  return {
    webSearch: resolveBooleanValue(lower?.webSearch, higher?.webSearch, DEFAULTS.toolPermissions.webSearch, nullClearsValue),
    ...
    fileSystem: resolveBooleanValue(lower?.fileSystem, higher?.fileSystem, DEFAULTS.toolPermissions.fileSystem, nullClearsValue),
    externalApis: resolveBooleanValue(lower?.externalApis, higher?.externalApis, DEFAULTS.toolPermissions.externalApis, nullClearsValue),
  };
}
...
function mergePartialProfiles(
  ...
  result.notifications = mergeNotifications(lower.notifications, higher.notifications, options.nullClearsValue);
  result.privacy = mergePrivacy(lower.privacy, higher.privacy, options.nullClearsValue);
  result.modelPreferences = mergeModelPreferences(lower.modelPreferences, higher.modelPreferences, options.nullClearsValue);
  result.toolPermissions = mergeToolPermissions(lower.toolPermissions, higher.toolPermissions, options.nullClearsValue);

  // Custom instructions
  if (higher.customInstructions !== undefined) {
    result.customInstructions = higher.customInstructions === null && options.nullClearsValue ? undefined : higher.customInstructions;
 ...
 */
export function mergeProfiles(input: LayeredProfileInput, options: MergeOptions = {}): Profile {
  ...
  const allErrors = [...sharedValidation.errors, ...datadogValidation.errors, ...localValidation.errors];

  if (allErrors.length > 0) {
    const errorDetails = allErrors.map((e) => `[${e.source}] ${e.field}: ${e.message} (got: ${JSON.stringify(e.value)})`).join('\n');
    throw new Error(`Profile validation failed:\n${errorDetails}`);
  }

  // Merge in precedence order: shared -> datadog -> local
  const merged = mergePartialProfiles(mergePartialProfiles(input.shared, input.datadog, opts), input.local, opts);
  ...
  if (!finalValidation.valid) {
    const errorDetails = finalValidation.errors.map((e) => `[${e.source}] ${e.field}: ${e.message}`).join('\n');
