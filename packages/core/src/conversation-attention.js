...
function resolveLegacyConversationAttentionStatePath(options) {
    return join(getConversationAttentionStateRoot(options.stateRoot), 'pi-agent', 'state', 'conversation-attention', `${options.profile}.json`);
}
...
function normalizeRecord(value, fallbackConversationId) {
    ...
    if (typeof record.acknowledgedMessageCount !== 'number' ||
