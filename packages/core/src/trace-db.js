...
function resolveTraceTableMaxRows() {
...
function pruneTraceTable(db, table, maxRows = resolveTraceTableMaxRows()) {
  db.prepare(
    `
    DELETE FROM ${table}
    WHERE id IN (
...
export function querySessionSuggestedPointerIds(sessionId) {
  try {
    const db = getTraceDb();
    const rows = db.prepare(`SELECT pointer_ids FROM trace_suggested_context WHERE session_id = ? AND pointer_ids != ''`).all(sessionId);
    const ids = new Set();
