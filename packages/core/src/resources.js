...
function collectLayerDirs(layers, relativePath) {
  const dirs = layers.map((layer) => existingDir(join(layer.agentDir, relativePath))).filter((value) => value !== undefined);
  return dedupe(dirs);
}
function collectLayerFiles(layers, relativePath) {
  const files = layers.map((layer) => existingFile(join(layer.agentDir, relativePath))).filter((value) => value !== undefined);
  return dedupe(files);
}
