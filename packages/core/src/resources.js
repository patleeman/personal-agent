...
import { getDurableAgentFilePath, getDurableProfileDir as getDurableRuntimeConfigDir, getDurableProfileModelsFilePath as getDurableRuntimeModelsFilePath, getDurableProfilesDir as getCanonicalRuntimeConfigRoot, getDurableProfileSettingsFilePath as getDurableRuntimeSettingsFilePath, getDurableSkillsDir, getDurableTasksDir, getLocalProfileDir as getCanonicalLocalProfileDir, getStateRoot, getSyncRoot, getVaultRoot, } from './runtime/paths.js';
import { renderSystemPromptTemplate } from './system-prompt-template.js';
const PACKAGE_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '../../..');
function buildVaultRootAppendSystemChunk(vaultRoot = getVaultRoot()) {
    return [
...
function isRemotePackageSource(value) {
...
function looksLikeExplicitLocalPath(value) {
    return (value === '.' ||
        value === '..' ||
        value.startsWith('./') ||
        value.startsWith('../') ||
...
function normalizePackageSource(value, baseDir) {
...
function extractPackageSource(entry) {
...
export function resolveLocalProfileDir(options = {}) {
...
export function resolveLocalProfileSettingsFilePath(options = {}) {
...
export function resolveRuntimeSettingsFilePath(runtimeScope, options = {}) {
...
export function resolveRuntimeModelsFilePath(runtimeScope, options = {}) {
    validateRuntimeScopeName(runtimeScope || 'shared');
    return getDurableRuntimeModelsFilePath(runtimeScope || 'shared', resolveRuntimeConfigRoot(options));
}
function readConfiguredPackageEntries(settingsPath) {
...
export function readPackageSourceTargetState(target, options = {}) {
...
export function installPackageSource(options) {
    const settingsPath = resolveLocalProfileSettingsFilePath(options);
...
export function getRepoRoot(explicitRepoRoot) {
    const value = explicitRepoRoot ?? process.env.PERSONAL_AGENT_REPO_ROOT ?? PACKAGE_ROOT;
    return resolve(value);
}
export function getRepoDefaultsAgentDir(explicitRepoRoot) {
...
function resolveRuntimeConfigRoot(options = {}) {
...
function getRuntimeConfigDir(runtimeScope, options = {}) {
    return getDurableRuntimeConfigDir(runtimeScope || 'shared', resolveRuntimeConfigRoot(options));
}
export function listRuntimeScopes(options = {}) {
    void options;
    return ['shared'];
}
function collectLayerDirs(layers, relativePath) {
    const dirs = layers
        .map((layer) => existingDir(join(layer.agentDir, relativePath)))
        .filter((value) => value !== undefined);
    return dedupe(dirs);
}
function collectLayerFiles(layers, relativePath) {
    const files = layers
        .map((layer) => existingFile(join(layer.agentDir, relativePath)))
        .filter((value) => value !== undefined);
    return dedupe(files);
}
function resolveConfiguredInstructionFiles() {
    return dedupe(readMachineInstructionFiles().flatMap((path) => {
        const file = existingFile(path);
        return file ? [file] : [];
    }));
}
function collectConfiguredSkillDirs(rootDir) {
    const directSkillFiles = [existingFile(join(rootDir, 'SKILL.md')), existingFile(join(rootDir, 'INDEX.md'))].filter((value) => value !== undefined);
...
function discoverExtensionEntries(extensionDir) {
...
function discoverFilesWithExtensions(rootDir, extensions) {
...
function validateRuntimeScopeName(runtimeScope) {
    if (!/^[a-zA-Z0-9][a-zA-Z0-9-_]*$/.test(runtimeScope)) {
        throw new Error(`Invalid runtime scope name "${runtimeScope}". ` + 'Runtime scope names may only include letters, numbers, dashes, and underscores.');
    }
}
function resolveSharedVaultAgentFile(options = {}) {
    return existingFile(getDurableAgentFilePath(resolveVaultRoot(options)));
}
function resolveDurableAgentFiles(_runtimeScope, options = {}) {
    const sharedAgent = resolveSharedVaultAgentFile(options);
    return sharedAgent ? [sharedAgent] : [];
}
function resolveDurableSettingsFiles(_runtimeScope, options = {}) {
    const output = [];
...
function resolveDurableModelsFiles(_runtimeScope, options = {}) {
    const output = [];
...
export function resolveRuntimeResources(name, options = {}) {
...
export function mergeJsonFiles(paths) {
    let merged = {};
    for (const path of paths) {
...
function combineMarkdownChunks(chunks, separator = '\n\n---\n\n') {
    return chunks
        .map((chunk) => chunk.trim())
        .filter((text) => text.length > 0)
        .join(separator);
}
function combineMarkdownFiles(paths) {
    return combineMarkdownChunks(paths.map((path) => readFileSync(path, 'utf-8')));
}
function readRuntimeLastChangelogVersion(settingsPath) {
    if (!existsSync(settingsPath)) {
        return undefined;
    }
    try {
        ...
        const value = parsed.lastChangelogVersion;
...
function mergeMaterializedSettings(settingsFiles, targetSettingsPath) {
...
function readSkillPromptSummary(skillDir) {
    ...
    try {
        ...
        const description = [record.description, record.summary, metadata.summary, metadata.description].find((candidate) => typeof candidate === 'string' && candidate.trim().length > 0);
...
function listAdditionalSkillPromptSummaries(resources) {
...
export function materializeRuntimeResourcesToAgentDir(resources, agentDir) {
    const targetDir = resolve(agentDir);
    mkdirSync(targetDir, { recursive: true });
    const writtenFiles = [];
    const writeOrRemove = (fileName, content) => {
    ...
    const materializedSettings = resources.settingsFiles.length > 0 ? mergeMaterializedSettings(resources.settingsFiles, join(targetDir, 'settings.json')) : null;
...
export function getExtensionDependencyDirs(resources) {
...
export function buildPiResourceArgs(resources, options = {}) {
    const args = [];
