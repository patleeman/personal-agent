...
function normalizeTags(values) {
    return [...new Set((values ?? []).map((value) => normalizeTag(String(value))).filter((value) => value.length > 0))].sort((left, right) => left.localeCompare(right));
}
...
function buildProjectTasksMarkdown(tasks) {
    ...
    return tasks
        ...
        const statusSuffix = !done && task.status !== 'pending' && task.status !== 'todo' && task.status !== 'in_progress' ? ` (status: ${task.status})` : '';
...
function copyLegacyNodeCandidate(candidate, nodesDir, existingKinds = []) {
    ...
    const title = readOptionalString(candidate.frontmatter.title) ??
        candidate.project?.title ??
        extractMarkdownTitle(candidate.body) ??
        humanizeId(candidate.id);
    const summary = readOptionalString(candidate.frontmatter.summary) ??
...
function collectLegacyCandidates(options = {}) {
    ...
    if (existsSync(skillsDir)) {
        for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
            if (!entry.isDirectory())
                continue;
            const indexPath = LEGACY_SKILL_FILE_NAMES.map((fileName) => join(skillsDir, entry.name, fileName)).find((filePath) => existsSync(filePath));
            if (!indexPath)
                continue;
            const parsed = parseLegacyMarkdown(indexPath);
    ...
    if (existsSync(projectsDir)) {
        for (const entry of readdirSync(projectsDir, { withFileTypes: true })) {
            ...
            if (!existsSync(indexPath) || !existsSync(statePath))
                continue;
            const parsed = parseLegacyMarkdown(indexPath);
            output.push({
