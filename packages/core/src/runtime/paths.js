...
function readMachineConfigRuntimeOverrides() {
    ...
    try {
        ...
        const record = parsed;
        const vaultRoot = typeof record.vaultRoot === 'string' && record.vaultRoot.trim().length > 0 ? expandHomePath(record.vaultRoot.trim()) : undefined;
...
export function validateStatePathsOutsideRepo(paths, repoRoot = process.cwd()) {
    ...
    if (violations.length > 0) {
        throw new Error(`Runtime state paths must be outside repository:\n${violations.join('\n')}\n\n` +
