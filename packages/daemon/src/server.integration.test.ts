...
describe('daemon IPC integration', () => {
  ...
  it('pulls queued deferred follow-ups', async () => {
    daemon = new PersonalAgentDaemon(config);
    await daemon.start();
    ...
    const emitResponse = await sendRequest(socketPath, {
      ...
      event: {
        id: `evt_${randomUUID()}`,
        version: 1,
        type: 'gateway.deferred-followup.ready',
        source: 'module:deferred-followups',
        timestamp: new Date().toISOString(),
        payload: {
          id: 'resume-1',
          gateway: 'telegram',
          conversationId: '123::thread:22',
          sessionFile: '/tmp/sessions/123.thread-22.jsonl',
    ...
    for (let attempt = 0; attempt < 10; attempt += 1) {
      ...
      const followUps = (pullResponse.result as { followUps?: unknown[] } | undefined)?.followUps ?? [];
    ...
    expect(pullResponse?.ok).toBe(true);
    const pulled = (pullResponse?.result as { followUps: Array<Record<string, unknown>> }).followUps;
    expect(pulled.length).toBe(1);
    expect(pulled[0]).toMatchObject({
      id: 'resume-1',
      gateway: 'telegram',
    ...
    expect(secondPull.ok).toBe(true);
    expect((secondPull.result as { followUps: unknown[] }).followUps).toEqual([]);
  });
