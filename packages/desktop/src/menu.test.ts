...
describe('buildDesktopApplicationMenuTemplate', () => {
  it('builds a macOS app menu with app-specific actions', () => {
    ...
    expect(template.map((item) => item.label)).toEqual(['Personal Agent', 'File', 'Edit', 'View', 'Window']);
    ...
    expect(editMenu?.submenu).toEqual(
      expect.arrayContaining([expect.objectContaining({ label: 'Find on Page', accelerator: 'CommandOrControl+F' })]),
    );
    ...
    expect(windowMenu?.submenu).toEqual(expect.arrayContaining([expect.objectContaining({ label: 'Hide Window' })]));
  });
  ...
  it('builds a non-mac file menu with quit actions', () => {
    ...
    expect(viewMenu?.submenu).toEqual(expect.arrayContaining([expect.objectContaining({ role: 'toggleDevTools' })]));
  });
});
