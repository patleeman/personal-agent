...
const desktopBridge = {
  ...
  readAlerts: () => ipcRenderer.invoke(`${CHANNEL_PREFIX}:read-alerts`),
  acknowledgeAlert: (alertId: string) => ipcRenderer.invoke(`${CHANNEL_PREFIX}:acknowledge-alert`, alertId),
  dismissAlert: (alertId: string) => ipcRenderer.invoke(`${CHANNEL_PREFIX}:dismiss-alert`, alertId),
  snoozeAlert: (input: { alertId: string; delay?: string; at?: string }) => ipcRenderer.invoke(`${CHANNEL_PREFIX}:snooze-alert`, input),
