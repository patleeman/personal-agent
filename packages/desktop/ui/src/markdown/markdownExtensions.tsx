...
export function buildMentionLookup(items: MentionItem[] | null | undefined): Map<string, MentionItem[]> {
  ...
  for (const item of items) {
    ...
    const existing = lookup.get(item.id);
    if (existing) {
      existing.push(item);
    } else {
...
function renderMentionFragment(mention: string, index: number, lookup: Map<string, MentionItem[]>): ReactNode {
...
function renderTextWithMentionLinks(text: string, options: { lookup: Map<string, MentionItem[]> }): ReactNode[] {
  return splitMentionFragments(text).map((fragment, index) => {
    if (fragment.mention) {
      return renderMentionFragment(fragment.text, index, options.lookup);
    }
