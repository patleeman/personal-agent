...
func newServer(runDir, piPath, sessionFile, cwd, agentDir string, listener net.Listener) (*server, error) {
...
func (s *server) startPi() error {
	...
	cmd.Dir = s.cwd
	if strings.TrimSpace(s.agentDir) != "" {
		cmd.Env = append(os.Environ(), fmt.Sprintf("PI_CODING_AGENT_DIR=%s", s.agentDir))
	}
...
func runServer(runDir, piPath, sessionFile, cwd, agentDir, listenAddr string) error {
	...
	server, err := newServer(runDir, piPath, sessionFile, cwd, agentDir, listener)
	if err != nil {
		_ = listener.Close()
		return err
	}
...
func main() {
	...
	case "launch":
		...
		agentDir := flags.String("agent-dir", "", "Path to Pi agent runtime directory")
		_ = flags.Parse(os.Args[2:])
		err = launchServer(launchOptions{RunDir: *runDir, PiPath: *piPath, SessionFile: *sessionFile, Cwd: *cwd, AgentDir: *agentDir})
	case "server":
		...
		_ = flags.Parse(os.Args[2:])
		err = runServer(*runDir, *piPath, *sessionFile, *cwd, *agentDir, *listen)
	case "list-dir":
