...
describe('registerExtensionRoutes', () => {
  ...
  it('invokes runtime extension backend actions', async () => {
    ...
    writeFileSync(
      ...
      `export async function saveTask(input, ctx) { await ctx.storage.put('tasks/one', input); return { saved: await ctx.storage.get('tasks/one'), automationsList: typeof ctx.automations.list, runsStart: typeof ctx.runs.start, vaultRead: typeof ctx.vault.read }; }`,
    );
    ...
    expect(res.json).toHaveBeenCalledWith({
      ok: true,
      result: { saved: { title: 'Ship it' }, automationsList: 'function', runsStart: 'function', vaultRead: 'function' },
    });
  });
