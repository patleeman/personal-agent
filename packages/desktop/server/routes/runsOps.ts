...
import { DurableRunCapabilityInputError, markDurableRunAttentionCapability } from '../automation/durableRunCapability.js';
...
export function registerRunsOpsRoutes(router: Pick<Express, 'get' | 'post' | 'patch'>): void {
  router.patch('/api/runs/:id/attention', async (req, res) => {
    try {
      ...
      res.json(
        await markDurableRunAttentionCapability({
          runId: req.params.id,
          read,
        }),
      );
