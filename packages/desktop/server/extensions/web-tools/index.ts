  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (exaApiKey) {
        try {
          ...
          if (!response.ok) {
            ...
            const header = `Exa Search | Page ${page} | Results ${resultStart}-${resultStart + results.length - 1} | Use page: ${
              page + 1
            } for more results\n\n`;
      ...
      try {
        ...
        const resultStart = offset + 1;
        const output = results
          .map((r, i) => `--- Result ${resultStart + i} ---\nTitle: ${r.title}\nURL: ${r.url}\nSnippet: ${r.snippet}`)
          .join('\n\n');

        const header = `DuckDuckGo Search | Page ${page} | Results ${resultStart}-${resultStart + results.length - 1} | Use page: ${
          page + 1
        } for more results\n\n`;
