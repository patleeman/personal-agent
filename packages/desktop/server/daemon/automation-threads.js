...
import { getStoredAutomation, setStoredAutomationThreadBinding, } from './automation-store.js';
function readOptionalString(value) {
    const normalized = value?.trim();
    return normalized && normalized.length > 0 ? normalized : undefined;
}
function persistSessionFile(manager) {
    const persistable = manager;
    persistable._rewriteFile?.();
    persistable.flushed = true;
}
function desiredAutomationThreadTitle(task) {
    const title = task.title.trim() || task.id;
    return `Automation: ${title}`;
}
function resolveStoredSessionsRoot(stateRoot) {
...
function resolveExistingSessionFile(input) {
    ...
    const match = listStoredSessions({ sessionsDir: resolveStoredSessionsRoot(input.stateRoot) }).find((session) => session.id === conversationId);
    if (!match) {
        return {
            conversationId,
        };
...
function ensureDedicatedThread(task, options) {
...
function ensureExistingThread(task, options) {
...
export function ensureAutomationThread(taskId, options = {}) {
...
export function resolveAutomationThreadTitle(task) {
    if (task.threadMode === 'none') {
        return undefined;
    }
    return desiredAutomationThreadTitle(task);
}
export function normalizeAutomationThreadModeForSelection(value) {
