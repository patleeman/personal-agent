...
function toTimestampKey(timestamp) {
...
function extractIsRunning(value) {
    if (!isRecord(value)) {
        return undefined;
    }
...
async function resolveTaskConversation(runtime, task) {
    if (task.threadMode && task.threadMode !== 'none' && task.threadSessionFile) {
        ...
        return (extractConversationId(resumed) ??
...
export async function runTaskInIsolatedPi(request) {
