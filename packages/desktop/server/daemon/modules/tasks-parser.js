...
function toTaskIdFromFile(filePath) {
...
function readOptionalString(attributes, key) {
...
function readEnabled(attributes) {
...
function readTimeoutSeconds(attributes, defaultTimeoutSeconds) {
...
function parseCronNumber(raw, min, max, label, allowSunday7 = false) {
    if (!/^\d+$/.test(raw)) {
...
function normalizeCronValue(value, allowSunday7) {
    if (allowSunday7 && value === 7) {
        return 0;
    }
    return value;
}
function parseCronField(raw, min, max, label, allowSunday7 = false) {
    const trimmed = raw.trim();
    if (trimmed.length === 0) {
...
export function parseTaskDefinition(options) {
