import { createReadyDeferredResume, getTaskCallbackBinding, loadDeferredResumeState, resolveDeferredResumeStateFile, saveDeferredResumeState, upsertAlert, } from '@personal-agent/core';
import { randomUUID } from 'crypto';
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'fs';
import { dirname, join, resolve } from 'path';
import { appendAutomationActivityEntry, deleteStoredAutomation, ensureLegacyTaskImports, getStoredAutomation, listStoredAutomations, loadAutomationRuntimeStateMap, loadAutomationSchedulerState, saveAutomationRuntimeStateMap, saveAutomationSchedulerState, } from '../automation-store.js';
...
import { appendDurableRunEvent, createDurableRunManifest, createInitialDurableRunStatus, resolveDurableRunPaths, resolveDurableRunsRoot, resolveRuntimeDbPath, saveDurableRunCheckpoint, saveDurableRunManifest, saveDurableRunStatus, scanDurableRun, } from '../runs/store.js';
import { cronMatches } from './tasks-parser.js';
import { runTaskInIsolatedPi } from './tasks-runner.js';
import { createEmptyTaskState } from './tasks-store.js';
const MISSED_RUN_EXAMPLE_LIMIT = 5;
function resolveValidNow(input) {
    return Number.isFinite(input.getTime()) ? input : new Date();
}
function normalizeIntegerConfig(value, fallback, minimum) {
...
function toRunIdTimestamp(value) {
    return value.replace(/[:.]/g, '-');
}
function createScheduledTaskRunId(taskId, startedAt) {
...
function copyTaskRunLogToDurableOutput(logPath, outputLogPath) {
...
function toMinuteKey(at) {
    const rounded = new Date(at);
    rounded.setSeconds(0, 0);
    return rounded.toISOString();
}
function toMinuteStart(at) {
    const rounded = new Date(at);
    rounded.setSeconds(0, 0);
    return rounded;
}
function toNextMinuteStart(after) {
...
function summarizeMissedCronRuns(expression, evaluatedAt, currentTime) {
...
function resolveCatchUpScheduledAt(task, missedRuns, currentTime) {
...
function toMissedTaskActivitySummary(taskId, missedRunCount) {
    if (missedRunCount === 1) {
...
function ensureTaskRecord(taskState, task) {
...
export function createTasksModule(config, dependencies = {}) {
    const now = () => resolveValidNow(dependencies.now?.() ?? new Date());
    const runTask = dependencies.runTask ?? ((request) => runTaskInIsolatedPi(request));
    ...
    const formatTaskCallbackPrompt = (task, status, details) => {
        const lines = [
            status === 'success'
    ...
    const deliverTaskCallbackWakeup = async (task, status, context, details) => {
    ...
    const writeMissedTaskActivity = (task, context, details) => {
        context.logger.info([
        ...
        try {
            const activity = appendAutomationActivityEntry(task.id, {
                kind: 'missed',
            ...
            context.logger.warn(`failed to record missed task activity id=${task.id}: ${error.message}`);
        }
    };
    const createDurableTaskRunRecord = async (task, record, startedAt, runIdOverride) => {
    ...
    const executeTaskRun = async (task, record, context, controller, options = {}) => {
    ...
    const startTaskRun = (task, record, context, options = {}) => {
        ...
        const runPromise = executeTaskRun(task, record, context, controller, options)
            ...
            try {
                ...
                context.logger.warn(`failed to record task start failure id=${task.id}: ${activityError.message}`);
            }
        })
            .finally(() => {
    ...
    const handleRequestedTaskRun = async (payload, context) => {
        ...
        try {
            ...
            context.logger.warn(`failed to start requested task run id=${taskId}: ${error.message}`);
        }
    };
    const reconcileDeletedTaskState = (activeTaskKeys) => {
    ...
    const reapResolvedOneTimeTasks = (currentTimeMs, context) => {
        if (reapAfterDays <= 0) {
            return;
        }
        ...
        for (const [key, record] of Object.entries(taskState.tasks)) {
            ...
            try {
                ...
                context.logger.warn(`failed to reap resolved one-time task id=${record.id}: ${error.message}`);
            }
        }
    };
            ...
            for (const task of parsedTasks) {
                ...
                if (activeRuns.has(task.key)) {
                    ...
                    try {
                        ...
                        context.logger.warn(`failed to alert skipped task id=${task.id}: ${error.message}`);
                    }
                    continue;
                }
                startTaskRun(task, record, context);
