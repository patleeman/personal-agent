import { createProjectActivityEntry, getAlert, getConversationProjectLink, readSessionConversationId, setActivityConversationLinks, upsertAlert, writeProfileActivityEntry, } from '@personal-agent/core';
...
function buildWakeupSummary(record) {
    switch (record.kind) {
        case 'reminder':
...
function buildWakeupDetails(record) {
    const lines = [
        `Kind: ${record.kind}`,
...
function buildAlertTitle(record) {
    switch (record.kind) {
        case 'reminder':
...
function buildAlertBody(record) {
...
export function buildDeferredResumeActivityId(record) {
    return `deferred-resume-fired-${sanitizeIdSegment(record.id)}`;
}
export function buildDeferredResumeAlertId(record) {
...
function shouldRefreshExistingAlert(existing, next) {
