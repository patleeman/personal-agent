import { createReadyDeferredResume, loadDeferredResumeState, resolveDeferredResumeStateFile, saveDeferredResumeState, } from '@personal-agent/core';
...
import { loadDurableRunCheckpoint, resolveDurableRunPaths, saveDurableRunCheckpoint, scanDurableRun, } from './store.js';
...
function buildWakeupPrompt(run) {
    ...
    lines.push('', 'Recent log tail:', readLogTail(logPath, 60), '', 'Use run get/logs if you need more detail. Then continue from this point.');
    return lines.join('\n');
}
function buildWakeupTitle(run) {
    const taskSlug = readTaskSlug(run);
...
function buildWakeupId(runId) {
    return `background-run-${sanitizeIdSegment(runId)}`;
}
function getBackgroundRunCallbackBinding(run) {
...
export function getBackgroundRunCallbackDelivery(run) {
...
function shouldDeliverCallbackForStatus(status) {
