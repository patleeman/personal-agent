...
import { loadDurableRunCheckpoint, resolveDurableRunPaths, saveDurableRunCheckpoint, scanDurableRun, scanDurableRunsForRecovery, } from './store.js';
...
function resolveEligibleBackgroundRun(run) {
    ...
    const resumeParentOnExit = payload.resumeParentOnExit === true || (isRecord(payload.metadata) && payload.metadata.resumeParentOnExit === true);
...
function readRunLogTail(logPath, maxLines) {
...
function describeRunCommand(run) {
    const payload = readCheckpointPayload(run.checkpoint);
...
function buildGenericSingleRunPrompt(run) {
...
function buildGenericBatchPrompt(runs) {
...
function buildBackgroundRunResultPrompt(runs) {
    const orderedRuns = sortRuns(runs);
    if (orderedRuns.length === 1) {
        return buildGenericSingleRunPrompt(orderedRuns[0]);
    }
    return buildGenericBatchPrompt(orderedRuns);
}
function createBackgroundRunResultBatchId(sessionFile, runIds) {
...
function markRunsSurfaced(runsRoot, runs, batchId, surfacedAt) {
