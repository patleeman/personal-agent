...
function readCallbackConversation(run) {
...
function copyReplayMetadata(run) {
    const metadata = { ...readMetadata(run) };
...
function canReplayBackgroundRun(run) {
    return run.manifest?.kind === 'background-run' || run.manifest?.kind === 'raw-shell';
}
export function buildRerunBackgroundRunInput(run) {
...
export function buildFollowUpBackgroundRunInput(run, prompt) {
    if (run.manifest?.kind !== 'background-run') {
