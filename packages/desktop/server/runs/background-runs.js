...
import { buildBackgroundAgentArgv } from '../background-run-agent.js';
import { scheduleRun } from './schedule-run.js';
import { appendDurableRunEvent, createInitialDurableRunStatus, loadDurableRunCheckpoint, loadDurableRunManifest, loadDurableRunStatus, saveDurableRunCheckpoint, saveDurableRunStatus, } from './store.js';
...
function ensureCommandSpec(input) {
    ...
    const definedSpecs = [argv ? 'argv' : null, shellCommand ? 'shellCommand' : null, agent ? 'agent' : null].filter((value) => value !== null);
...
function appendOutputLog(path, text) {
    mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
    appendFileSync(path, text, 'utf-8');
}
export function createBackgroundRunId(taskSlug, createdAt) {
    const nonce = Math.random().toString(16).slice(2, 10);
    return ['run', sanitizeIdSegment(taskSlug, 'background'), toTimestampKey(createdAt), nonce].join('-');
}
export async function createBackgroundRunRecord(runsRoot, input) {
 ...
 */
function buildScheduleRunInputFromBackgroundRun(input, argv, shellCommand, agent) {
...
export async function finalizeBackgroundRun(input) {
    const manifest = loadDurableRunManifest(input.runPaths.manifestPath);
