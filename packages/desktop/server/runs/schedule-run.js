...
import { appendDurableRunEvent, createDurableRunManifest, createInitialDurableRunStatus, resolveDurableRunPaths, resolveDurableRunsRoot, saveDurableRunCheckpoint, saveDurableRunManifest, saveDurableRunStatus, } from './store.js';
// ---------------------------------------------------------------------------
// Derived values
// ---------------------------------------------------------------------------
/**
...
function resolveLoopOptions(input) {
    ...
    const maxIterations = Number.isSafeInteger(input.maxIterations) && input.maxIterations > 0
        ? Math.min(MAX_LOOP_ITERATIONS, input.maxIterations)
        : undefined;
    const retryAttempts = Number.isSafeInteger(input.retry?.attempts) && input.retry?.attempts > 0
