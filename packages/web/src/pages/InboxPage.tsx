        ...
        {!loading && !error && activity?.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="text-xs text-secondary mt-1">
              Activity is created when scheduled tasks run or deferred resumes fire.
            </p>
          </div>
        )}
            ...
            {activity.map((entry) => {
              ...
              return (
                <Link
                  key={entry.id}
                  to={`/inbox/${entry.id}`}
                  className="flex items-start gap-4 px-4 py-3.5 -mx-2 rounded-lg hover:bg-surface transition-colors group"
                >
                  {/* Kind dot */}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-primary group-hover:text-white leading-snug">
                      {entry.summary}
                    </p>
                    <div className="flex items-center gap-3 mt-1.5">
                      <span className={`text-2xs px-1.5 py-0.5 rounded font-medium ${meta.color}`}>
                        {meta.label}
                      </span>
                  ...
                  {/* Arrow */}
                  <span className="text-dim group-hover:text-secondary transition-colors text-sm mt-0.5">
                    →
                  </span>
                </Link>
              );
