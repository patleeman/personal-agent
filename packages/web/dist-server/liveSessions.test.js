...
describe('live session subscriptions', () => {
    ...
    it('deduplicates reordered live compaction windows against persisted history', () => {
        ...
        const toolABlocks = blocks.filter((block) => (block.type === 'tool_use' && block.toolCallId === 'tool-a'));
        ...
        expect(blocks.filter((block) => (block.type === 'thinking' && block.text === 'Live-only planning'))).toHaveLength(1);
