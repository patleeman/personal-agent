...
import { buildSessionContext, calculateContextTokens, estimateTokens, getLatestCompactionEntry, migrateSessionEntries, parseSessionEntries, } from '@mariozechner/pi-coding-agent';
...
const SEGMENT_ORDER = ['system', 'user', 'assistant', 'tool', 'summary', 'other'];
...
function addWeight(target, key, value) {
    if (value <= 0) {
        return;
    }
    target[key] += value;
}
function measureRichContentWeight(content) {
...
function estimateMessageWeights(message) {
...
function estimateContextWeights(messages) {
    const weights = createEmptyWeights();
...
function scaleWeightsToSegments(weights, totalTokens) {
    if (totalTokens <= 0) {
        return [];
    }
...
function hasPostCompactionUsage(branchEntries, compactionIndex) {
...
function estimateSessionContextTokens(messages) {
...
export function readSessionContextUsageFromFile(filePath) {
    const fileEntries = parseSessionEntries(readFileSync(filePath, 'utf-8'));
