...
interface LocalCheckpointCommitMetadata {
  commitSha: string;
  shortSha: string;
  subject: string;
  body?: string;
...
function readRequiredCheckpointString(value: string | undefined, label: string): string {
...
function normalizeCheckpointPaths(cwd: string, values: string[]): string[] {
  const seen = new Set<string>();
...
function parseCheckpointDiffSections(rawPatch: string) {
  ...
  return sections.map((section) => {
    ...
    let status: 'added' | 'modified' | 'deleted' | 'renamed' | 'copied' = oldPath === '/dev/null'
      ? 'added'
      : newPath === '/dev/null'
        ? 'deleted'
        : previousPath
...
function createConversationCheckpointCommit(options: { cwd: string; message: string; paths: string[] }) {
...
export async function createDesktopConversationCheckpoint(input: {
  conversationId: string;
  message: string;
  paths: string[];
}) {
