...
import { listProjectIndex, readProjectDetailFromProject, readProjectSource } from '../projects/projects.js';
...
function buildProjectText(detail: ReturnType<typeof readProjectDetailFromProject>): string[] {
  return [
    ...
    ...detail.childPages.flatMap((page) => [page.title, page.summary, page.body]),
  ].filter((value) => value.trim().length > 0);
}

function readProjectDocuments(options: { repoRoot: string; profile: string }): NodeDocument[] {
  return listProjectIndex({ repoRoot: options.repoRoot, profile: options.profile }).projects.flatMap((project) => {
