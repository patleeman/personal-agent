...
function isKeyValueTag(value: string): boolean {
...
function handleRouteError(err: unknown, res: { status: (code: number) => { json: (value: unknown) => void } }, fallbackStatus = 500): void {
  ...
  const status = message.startsWith('Unknown profile:')
    ? 400
    : message.startsWith('Page not found:') || message.startsWith('No page found with id:') || message.startsWith('Node not found:') || message.startsWith('No node found with id:')
      ? 404
...
export function registerNodeRoutes(
  ...
  for (const path of ['/api/pages', '/api/nodes']) {
  ...
  for (const path of ['/api/pages/views', '/api/nodes/views']) {
  ...
  for (const path of ['/api/pages/views/:viewId', '/api/nodes/views/:viewId']) {
  ...
  for (const path of ['/api/pages/:nodeId', '/api/nodes/:nodeId']) {
    router.get(path, (req, res) => {
      try {
        const profile = resolveRequestedProfile(req.query.viewProfile, context);
    ...
    router.patch(path, (req, res) => {
      try {
        ...
        const nextTags = Array.isArray(req.body?.tags)
          ...
          : null;
        if (nextTags) {
          const invalidTags = nextTags.filter((tag: string) => !isKeyValueTag(tag));
          if (invalidTags.length > 0) {
            throw new Error(`Tags must use key:value format. Invalid tags: ${invalidTags.join(', ')}`);
          }
        }
        updateUnifiedNode({
          id: nodeId,
