...
describe('conversation self-distill wakeups', () => {
  it('schedules one conservative wakeup per conversation and dedupes repeats', async () => {
    ...
    expect(first.resume.prompt).toContain('Self-distill this conversation with a high bar.');
    expect(first.resume.prompt).toContain('Allowed first-pass outcomes:');
    expect(first.resume.prompt).toContain('- no-op');
    expect(first.resume.prompt).toContain('- note update');
    expect(first.resume.prompt).toContain('- project update');
