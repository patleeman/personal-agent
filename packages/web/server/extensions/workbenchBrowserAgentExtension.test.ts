...
describe('workbench browser agent extension', () => {
  ...
  it('routes tools through the desktop host', async () => {
    ...
    const screenshot = await tools[1]!.execute('tool-3' as never, {} as never, undefined as never, undefined as never, ctx as never) as { content: Array<{ type: string; data?: string }> };
