...
describe('sync server helpers', () => {
  ...
  it('returns warnings when repo is missing and daemon is offline', async () => {
    ...
    expect(snapshot.warnings.some((warning: string) => warning.includes('not initialized'))).toBe(true);
    expect(snapshot.warnings.some((warning: string) => warning.includes('offline'))).toBe(true);
  });
