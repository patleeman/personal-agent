...
export function createRuntimeModelRegistry(authStorage: AuthStorage): ModelRegistry {
  return ModelRegistry.create(authStorage, join(getPiAgentRuntimeDir(), 'models.json'));
}

export function createModelRegistryForAuthFile(authFile: string): ModelRegistry {
  return ModelRegistry.create(AuthStorage.create(authFile), join(dirname(authFile), 'models.json'));
}
