...
- [ ] Finish the broader artifact model for task/job records.
- [x] Define activity/inbox records at an initial file-backed level.
- [ ] Define how deferred resume and scheduled tasks connect to artifacts and activity.
- [ ] Define the UI shell and attention model.
- [ ] Define how conversations pick up prior work across surfaces.
- [ ] Integrate daemon tasks and gateway output with the new model.
...
- the repo has one obvious way to read and write summary/plan artifacts
- artifact files remain plain text and git-friendly
- the minimum schema is small and easy to evolve
...
- define the left sidebar model:
  - inbox first
  - conversations ordered by attention
  - conversations needing attention bubbled to the top
- define the center panel as the primary Pi conversation panel
- define the right panel as the context stack for:
  - objective/intent
  - summary
  - plan
  - activities
...
- the default UI shell is clear enough to guide implementation without inventing a project-management UI
...
1. Define the minimal artifact shape for task/job records.
2. Connect daemon task results to artifact + activity output by default.
3. Decide how deferred resume should emit activity without becoming a scheduled task.
4. Define how activity bubbles related conversations into the inbox/sidebar attention model.
