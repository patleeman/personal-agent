...
- removed the unused inbox activity-count surface from the renderer/public route layer (`api.activityCount`, `/api/activity/count`, and the matching renderer desktop bridge exposure), since the product no longer calls that count endpoint after conversation/session attention summaries became the real notification source
- validation for the dead activity-count cleanup slice: targeted Vitest in `packages/web/server/routes/activity.test.ts` and `packages/web/src/api.desktop.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the app still boots with `window.personalAgentDesktop.readActivityCount === undefined`
