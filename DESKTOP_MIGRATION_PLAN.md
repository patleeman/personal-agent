...
- validation for this slice: targeted Vitest coverage in `packages/web/src/api.desktop.test.ts` and `packages/desktop/src/hosts/local-host-controller.test.ts`, eslint on touched files, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and a built Electron smoke check confirming the packaged app still launches and renders after the IPC wiring change
