...
- validation for this slice: targeted Vitest coverage for desktop bridge routing, local host controller routing, models routes, and conversation-plan preference helpers, plus eslint, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and built Electron smoke checks verifying the conversation-plan desktop bridge methods exist, return defaults/library/workspace state, and leave the Automations page rendering in local desktop mode
- current focus after this slice: continue replacing the remaining generic local API usage in local desktop product surfaces, especially file-backed artifacts/attachments and workspace/vault inspection flows
