...
- validation for this slice: targeted Vitest coverage in `packages/web/src/api.desktop.test.ts` and `packages/desktop/src/hosts/local-host-controller.test.ts`, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and agent-browser smoke checks against the built packaged Electron app confirming the local settings workspace panel, tools page, instructions page, and conversation navigation still render after the IPC wiring change
- current focus after this slice: continue replacing the remaining generic local API usage in local desktop product surfaces, especially file-backed artifacts/attachments, vault/workspace inspection flows, and any residual admin pages that still rely on generic local-api dispatch
