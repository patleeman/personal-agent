...
- validation for this slice: targeted Vitest coverage in `packages/web/server/automation/durableRunCapability.test.ts`, `packages/web/server/routes/runsOps.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, plus eslint, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and built Electron smoke checks confirming the packaged shell still renders while `markDurableRunAttention(...)` is exposed in the app and rejects missing run ids over the bridge as expected
