...
- current focus after this slice: clean the remaining UI/internal leftover `companion` naming that is no longer product-defining (`ChatView` compact layout name, note CSS class names, a few comments/tests/docs), then shift into startup/new-conversation perf cleanup on top of the simplified single-surface architecture
