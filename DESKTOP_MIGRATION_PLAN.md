...
- validation for this slice: targeted Vitest coverage for desktop bridge routing, local host controller routing, and packaged local-api module resolution, plus `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and agent-browser smoke checks against the built Electron app covering live-session creation, conversation model-preference reads/updates, and conversation cwd changes in local desktop mode
