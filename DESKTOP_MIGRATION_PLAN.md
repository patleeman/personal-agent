...
- validation for this slice: targeted Vitest coverage for desktop bridge routing, local host controller routing, provider OAuth desktop subscriptions, models routes, inbox capability routing, scheduled-task capability routing, conversation recovery, and packaged local-api module resolution, plus `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and agent-browser smoke checks against the built Electron app covering live-session creation, conversation model-preference reads/updates, conversation cwd changes, durable run bridge reads, task create/detail/edit flows, inbox bridge reads, and provider/settings bridge reads in local desktop mode
- current focus after this slice: continue replacing the remaining generic local API usage in local Settings/admin surfaces such as automation defaults/library/workspace and other non-remote-only operator panels
