...
- local desktop app startup now short-circuits the remote-only `/desktop-auth/session` probe instead of routing it through the generic local API path, so the packaged local app no longer spends startup work asking whether tailnet desktop sign-in is required when it is known not to be
- daemon managed-service lifecycle actions now explicitly reject in desktop runtime mode instead of silently behaving like legacy launchd/systemd admin surfaces, and local desktop companion/system views now present the daemon as an app-owned runtime rather than a restartable managed service
...
- validation for this slice: targeted Vitest coverage in `packages/web/server/automation/daemon.test.ts`, `packages/web/server/routes/daemon.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/web/src/companion/CompanionOpsPages.test.tsx`, plus eslint, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and built Electron smoke checks confirming the packaged shell still renders at `personal-agent://app/conversations/new` and exposes both `window.personalAgentDesktop.restartActiveHost` and `window.personalAgentDesktop.checkForUpdates`
- current focus after this slice: continue replacing or explicitly quarantining the few remaining local desktop generic-local-API fallbacks that are still meaningful, then shift into stricter startup/new-conversation perf work and codebase cleanup once the local desktop boundary is fully crisp
