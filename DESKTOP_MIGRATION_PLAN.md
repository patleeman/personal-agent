...
- validation for this slice: targeted Vitest coverage in `packages/web/server/conversations/conversationDeferredResumeCapability.test.ts`, `packages/web/server/routes/conversations.test.ts`, `packages/web/server/workspace/shellRunCapability.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, plus eslint, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and built Electron smoke checks confirming the packaged shell still renders while the new `runShellCommand(...)` bridge works in the app and the deferred-resume bridge methods are present
- current focus after this slice: continue replacing the remaining local desktop route-shaped flows, especially any residual admin/workspace or file/download surfaces that still lean on the generic local API path or custom-protocol fetches instead of typed bridge methods
