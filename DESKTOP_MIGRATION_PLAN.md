...
- validation for this slice: targeted Vitest coverage in `packages/web/server/automation/durableRunCapability.test.ts`, `packages/web/server/routes/runsOps.test.ts`, and `packages/web/src/api.desktop.test.ts`, plus eslint, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and built Electron smoke checks confirming the packaged shell still renders while `markDurableRunAttention(...)` is exposed in the app and the local startup path still opens directly into the packaged conversation UI
- current focus after this slice: continue replacing the last local desktop route-shaped flows that still matter to the product, and keep shrinking packaged-renderer dependence on generic local API/custom-protocol fetch fallbacks wherever there is still a meaningful active UI path
