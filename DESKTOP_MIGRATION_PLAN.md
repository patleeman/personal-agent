...
- validation for the remote-access cleanup slice: targeted Vitest coverage in `packages/web/server/routes/auth.test.ts`, `packages/web/server/routes/registerAll.smoke.test.ts`, `packages/web/src/api.desktop.test.ts`, `packages/web/src/apiBase.test.ts`, `packages/web/src/components/SystemContextPanel.test.tsx`, `packages/web/src/pages/SystemPage.test.tsx`, `packages/web/src/pages/SettingsPage.test.tsx`, `packages/desktop/src/hosts/local-host-controller.test.ts`, `packages/cli/src/ui-commands.test.ts`, and `packages/cli/src/ui-foreground.test.ts`, plus focused eslint, `npm --prefix packages/cli run build`, `npm --prefix packages/desktop run build`, and a built Electron smoke check confirming Settings still renders cleanly in the packaged shell
- current focus after this slice: rename or replace the remaining internal `companionAuth` storage/module terminology and prune dead `registerCompanion*` route helpers/tests that survived only as unused legacy scaffolding, then continue with startup/new-conversation perf cleanup on top of the simplified single-surface architecture
