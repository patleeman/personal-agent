...
- validation for this slice: targeted Vitest coverage in `packages/web/server/conversations/conversationSessionAssetCapability.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, plus eslint, `npm --prefix packages/web run build`, `npm --prefix packages/desktop run build`, and built Electron smoke checks confirming the packaged conversation shell still renders and existing conversations still open after the bootstrap/session-detail asset-inline change
- current focus after this slice: continue replacing the remaining local desktop route-shaped flows, especially residual workspace/admin surfaces like shell run plus any other conversation/file asset paths that still depend on custom-protocol fetches instead of typed bridge methods
