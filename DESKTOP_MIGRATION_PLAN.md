...
- removed the last dead HTTP runtime-action route registrations from the web server (`/api/application/restart`, `/api/application/update`, `/api/daemon/service/*`, and `/api/web-ui/service/*`), since those endpoints no longer have in-repo callers after the runtime panels and renderer bridge surface were pruned
- validation for the dead runtime-route cleanup slice: targeted Vitest in `packages/web/server/routes/system.routes.test.ts`, `packages/web/server/routes/daemon.test.ts`, and `packages/web/server/routes/webUi.test.ts`, focused eslint on the touched route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots
- removed the unused inbox activity-count surface end-to-end (`api.activityCount`, `/api/activity/count`, the renderer desktop bridge exposure, and the remaining local-host/local-api plumbing behind it), since the product no longer calls that count endpoint after conversation/session attention summaries became the real notification source
- validation for the dead activity-count cleanup slice: targeted Vitest in `packages/web/server/routes/activity.test.ts`, `packages/web/server/automation/inboxCapability.test.ts`, `packages/web/src/api.desktop.test.ts`, `packages/desktop/src/hosts/local-host-controller.test.ts`, and `packages/desktop/src/app-protocol.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the app still boots with `window.personalAgentDesktop.readActivityCount === undefined`
- removed the dead live-session-specific rename and context-usage surfaces (`api.renameSession`, `api.liveSessionContextUsage`, `PATCH /api/live-sessions/:id/name`, `GET /api/live-sessions/:id/context-usage`, and the matching desktop bridge/local-api plumbing), since the product now uses unified conversation rename flows plus `sessionDetail.contextUsage` instead of separate live-session-only helpers
- validation for the dead live-session metadata cleanup slice: targeted Vitest in `packages/web/server/routes/liveSessions.routes.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots
- removed the dead live-session stats surface end-to-end (`api.liveSessionStats`, `GET /api/live-sessions/:id/stats`, the dedicated desktop stats bridge, and the remaining local-api/host plumbing), since the product has no live callers for standalone session stats and already gets the context/session data it actually renders through the main conversation/session flows
- validation for the dead live-session stats cleanup slice: targeted Vitest in `packages/web/server/routes/liveSessions.routes.test.ts`, `packages/web/server/routes/registerAll.smoke.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots with `window.personalAgentDesktop.readLiveSessionStats === undefined`
- removed another dead post-tools cleanup cluster with no product callers: `api.installPackageSource`, `api.mcpServer`, `api.mcpTool`, `api.memoryFile`, `api.memoryFileSave`, plus the corresponding HTTP routes `/api/tools/packages/install`, `/api/tools/mcp/servers/:server`, `/api/tools/mcp/servers/:server/tools/:tool`, and `/api/memory/file`
- validation for the dead tools/memory detail cleanup slice: targeted Vitest in `packages/web/server/routes/tools.test.ts` and `packages/web/server/routes/memoryNotes.test.ts`, focused eslint on the touched API/route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots after the route/API removals
- removed the dead standalone conversation-plan defaults/library surface (`api.conversationPlanDefaults`, `api.updateConversationPlanDefaults`, `api.conversationPlanLibrary`, `api.updateConversationPlanLibrary`, the matching `/api/conversation-plans/defaults|library` routes, and the dedicated desktop bridge/local-api plumbing), since the product only consumes the combined workspace view now
- validation for the dead conversation-plan defaults/library cleanup slice: targeted Vitest in `packages/web/server/routes/models.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots with only the workspace bridge left
- removed the dead live-session list surface (`api.liveSessions`, `GET /api/live-sessions`, and the dedicated desktop live-session list bridge/local-api plumbing), since the product only uses the sidebar/session snapshots plus per-conversation live-session detail flows and no longer calls the standalone list endpoint
- validation for the dead live-session list cleanup slice: targeted Vitest in `packages/web/server/routes/liveSessions.routes.test.ts`, `packages/web/src/api.desktop.test.ts`, `packages/web/src/sessionSnapshot.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots with `window.personalAgentDesktop.readLiveSessions === undefined`
- removed the dead renderer/public delete surface for conversation artifacts and attachments (`api.deleteConversationArtifact`, `api.deleteConversationAttachment`, the matching `DELETE /api/conversations/:id/artifacts/:artifactId` and `DELETE /api/conversations/:id/attachments/:attachmentId` routes, and the dedicated desktop delete bridge/local-api plumbing), since the product has no in-repo callers for those delete actions and the remaining server-side cleanup/tooling paths call the shared capability layer directly
- validation for the dead artifact/attachment delete cleanup slice: targeted Vitest in `packages/web/server/routes/conversations.test.ts`, `packages/web/server/routes/registerAll.smoke.test.ts`, `packages/web/src/api.desktop.test.ts`, and `packages/desktop/src/hosts/local-host-controller.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots with `window.personalAgentDesktop.deleteConversationArtifact === undefined` and `deleteConversationAttachment === undefined`
- deferred resumes can now be scheduled to fire as explicit follow-up work instead of a normal prompt: the behavior now persists in deferred-resume state, the HTTP/desktop scheduling APIs accept it, live deferred-resume delivery respects it, the web `deferred_resume` agent extension accepts it, and the `/resume` slash command now supports `--follow-up`
- validation for the deferred-resume follow-up slice: targeted Vitest in `packages/core/src/deferred-resume.test.ts`, `packages/web/src/deferredResumeSlashCommand.test.ts`, `packages/web/server/automation/deferredResumes.test.ts`, `packages/web/server/conversations/conversationDeferredResumeCapability.test.ts`, `packages/web/server/routes/conversations.test.ts`, `packages/web/src/api.desktop.test.ts`, `packages/desktop/src/hosts/local-host-controller.test.ts`, and `packages/web/server/extensions/deferredResumeAgentExtension.test.ts`, focused eslint on the touched core/desktop/web files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming the local shell still boots with the deferred-resume desktop bridge still available
- measured the next packaged desktop startup/new-conversation drag point and trimmed it: the conversation page was still eagerly loading slash/mention discovery catalogs on mount even for plain prompts, including a roughly `39ms` local `/api/memory` read in the packaged shell; that work is now deferred until the user actually starts `/` or `@` discovery, while `/resume` and skill mentions still load on demand
- validation for the lazy conversation catalog perf slice: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming `/` still shows slash commands and `@a` still surfaces `agent-browser`
- removed one last clearly dead renderer/public/local-desktop surface after that: the unused shell-run public stack (`api.run`, the desktop `runShellCommand` bridge/preload/IPC/local-host plumbing, `/api/run`, and the now-unreferenced shell-run route/capability files) had no in-repo product callers, so it was deleted end to end instead of preserved as dead compatibility surface
- validation for the dead shell-run cleanup slice: targeted Vitest in `packages/web/src/api.desktop.test.ts`, `packages/desktop/src/hosts/local-host-controller.test.ts`, and `packages/web/server/routes/registerAll.smoke.test.ts`, focused eslint on the touched desktop/web route files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming `window.personalAgentDesktop.runShellCommand === 'undefined'` while `pickFolder` still exists
- trimmed another small first-visible-update hop on the local draft-create/open path: when a conversation is created from the draft page or via clear-and-recreate, the renderer now carries the already-known model/thinking selection through navigation state and skips the immediate follow-on `conversationModelPreferences` desktop read on that first open
- validation for the carried model-preference open-path trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a live conversation shell while an instrumented packaged check records `0` `readConversationModelPreferences(...)` calls during that first open
- trimmed another small packaged conversation-open read on the same path: saved conversation attachments are now fetched lazily only when the drawings picker actually opens instead of on every conversation open, so fresh desktop thread transitions no longer spend an attachment-list read on a hidden modal path
- validation for the lazy attachment-fetch trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, `npx electron-builder --config electron-builder.json5 --dir` after closing the running app, and a packaged-Electron smoke check confirming draft-create still lands in the saved conversation shell and `/drawings` still opens the drawings picker modal
- measured another remaining packaged create/open follow-on read and trimmed it: `readConversationDeferredResumes(...)` was still costing about `41ms` when called directly in the packaged shell, and fresh create/open paths were needlessly paying that read even though a brand-new conversation always starts with no wakeups; the route now carries an initial empty deferred-resume state so the first saved-conversation open can skip that immediate bridge read
- validation for the carried deferred-resume open-path trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a live conversation shell and gets a response with a measured route transition around `110ms`
- measured the next clearly non-essential packaged follow-on read after that and trimmed it: `readLiveSessionContext(...)` was still around `99ms` when called directly in the packaged shell, but on the fresh create/open path the immediate visible UI already has cwd from session meta and does not need branch/git context before the first prompt clears; git-context reads are now deferred until no initial prompt is pending or in flight
- validation for the deferred git-context warmup trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a live conversation shell, gets a response, and keeps the route transition around `110ms`
- measured another remaining first-response contention point in the packaged shell: repeated `session_meta_changed` app events were landing in short bursts during fresh create/send, and a direct local `readSessionMeta(...)` costs about `122ms`; the desktop app event handler now coalesces those metadata refreshes per conversation with a short trailing debounce instead of immediately firing a full meta read on every burst event
- validation for the coalesced session-meta refresh trim: targeted Vitest in `packages/web/src/sessionMetaRefreshScheduler.test.ts`, focused eslint on `packages/web/src/App.tsx` plus the new scheduler files, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a saved conversation shell and gets an assistant reply
- measured the next remaining cheap create/open follow-on read in the packaged shell: `readModels()` still costs about `70ms` locally and the saved conversation page was loading it immediately even while the carried initial prompt was still pending or in flight; model/default-model reads are now deferred on the saved conversation page until that first prompt clears, while the draft page still loads them eagerly
- validation for the deferred saved-page model-read trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a saved conversation shell, gets an assistant reply, and keeps the desktop bridge live
- measured another remaining first-response drag on the packaged local path: once a carried initial prompt starts writing blocks, the saved conversation page can immediately invalidate its file-backed conversation caches; in a fresh packaged run, a direct `readConversationBootstrap(...)` came in around `512ms` and `readSessionDetail(...)` around `58ms`, so those refreshes are now deferred until the carried prompt clears instead of competing with the first-turn hot path
- validation for the deferred file-refresh trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a saved conversation shell, gets an assistant reply, and keeps the desktop bridge live
- extended that same low-risk file-refresh guard after rechecking the packaged path directly from current `HEAD`: `readConversationBootstrap(...)` was still around `186ms` on a fresh local read, and file-backed refreshes could still wake up during the detached-start window before the prompt is fully owned by the live stream; conversation file refresh now also stays deferred while the carried prompt is still marked dispatching, not just after the in-flight ref flips
- validation for the dispatch-window file-refresh trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a saved conversation shell, gets an assistant reply, and keeps the route transition around `144ms`
- measured the next strongest remaining non-essential read on the packaged local path: direct `readLiveSessionContext(...)` was still about `1592ms` on a fresh local conversation, and the saved conversation page could start that git-context read during the detached-start dispatch window before the first live snapshot owned the turn; git-context reads are now deferred while the carried initial prompt is still marked dispatching, not just while the prompt payload or in-flight resend marker is present
- validation for the detached-start git-context trim: targeted Vitest in `packages/web/src/pages/ConversationPage.test.tsx`, focused eslint on `packages/web/src/pages/ConversationPage.tsx` and its test, `npm --prefix packages/desktop run build`, and a packaged-Electron smoke check confirming a draft prompt still opens into a saved conversation shell, gets an assistant reply, and keeps the route transition around `114ms`
- current focus after this slice: keep measuring the packaged local first-response / first-visible-update path for the next strongest cheap trim rather than widening cleanup scope again
