...
Scope:
- Only update repo-managed files under `profiles/assistant/agent/**`.
...
Authoritative files:
- Processed index (JSON): use the selector script output `indexPath` field (default runtime path: `~/.local/state/personal-agent/conversation-maintenance/assistant/processed-conversations.json`)
...
2. Generate the unprocessed-session list by running exactly:
   - `node scripts/conversation-maintenance/list-unprocessed-sessions.mjs --profile assistant --days 7 --timezone America/New_York`
...
8. Keep edits minimal, correct, and scoped to assistant profile files plus the resolved runtime processed index.
9. Git requirements:
