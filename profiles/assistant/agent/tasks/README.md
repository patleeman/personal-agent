...
- Task files are kept **adjacent** to `../memory/`.
- They are intentionally **not** stored inside `memory/` to reduce accidental edits during project note maintenance.

## Rules

1. Keep task files as `<name>.task.md` with valid frontmatter.
2. Do not place task files under `../memory/`.
