...
const IMAGE_QUALITY_VALUES = ['auto', 'low', 'medium', 'high'];
const IMAGE_BACKGROUND_VALUES = ['auto', 'opaque', 'transparent'];
const IMAGE_ACTION_VALUES = ['auto', 'generate', 'edit'];
const IMAGE_SOURCE_VALUES = ['none', 'latest', 'latest-user', 'latest-generated', 'recent'];
...
function readImageBlocks(content) {
...
function isValidImageSourceBase64(value) {
  if (value.length % 4 === 1 || !/^[A-Za-z0-9+/]+={0,2}$/.test(value)) {
    return false;
  }
  return Buffer.from(value, 'base64').length > 0;
}
function collectImageReferenceGroups(messages) {
  const groups = [];
  for (const message of messages) {
    ...
    if (role === 'user') {
      const images = readImageBlocks(message.content);
      if (images.length > 0) {
...
function findLastGroup(groups, kind) {
...
function inferSourceFromPrompt(prompt, groups) {
...
function inferActionFromPrompt(prompt, hasSourceImages) {
  if (!hasSourceImages) {
    return undefined;
  }
  const normalized = prompt.toLowerCase();
...
function buildImageGenerationPayload(input) {
  const content = input.sourceImages.map((image) => ({
    type: 'input_image',
    detail: 'auto',
...
function flushSseEvent(events, event, dataLines) {
...
export function parseImageGenerationSse(raw) {
  const events = [];
  let currentEvent = '';
  let dataLines = [];
...
export function createImageAgentExtension() {
  return (pi) => {
    pi.registerTool({
      name: 'image',
      label: 'Image',
