...
The extension mounts a tiny top-bar bootstrap component and also declares an `onEnableAction`. The backend `ensure` action is idempotent and de-duplicates concurrent calls, so the frontend bootstrap and enable hook can safely race during React Strict Mode or hot reload. When enabled, it creates the onboarding conversation, routes to it client-side, and then disables itself.
