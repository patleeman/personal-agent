...
const SCHEDULED_TASK_ACTION_VALUES = ['list', 'get', 'save', 'delete', 'validate', 'run'];
const SCHEDULED_TASK_TARGET_VALUES = ['background-agent', 'conversation'];
const SCHEDULED_TASK_THREAD_MODE_VALUES = ['dedicated', 'existing', 'none'];
const SCHEDULED_TASK_DELIVER_AS_VALUES = ['steer', 'followUp'];
...
function resolveOptionalScheduleAt(value) {
...
function formatTargetLabel(task) {
  return task.targetType === 'conversation' ? 'thread' : 'job';
}
function readConversationBehavior(value) {
  return value === 'steer' || value === 'followUp' ? value : undefined;
}
function readThreadMode(value) {
...
function formatTaskList(loaded) {
...
function formatTaskDetail(task, runtime, callbackBinding) {
  const threadDetail = buildScheduledTaskThreadDetail(task);
  const lines = [
    `Task @${task.id}`,
        ...
        try {
          ...
          switch (params.action) {
            case 'list': {
              ...
              return {
                content: [{ type: 'text', text: formatTaskList(loaded) }],
                details: {
                  action: 'list',
                  profile,
            ...
            case 'get': {
              ...
              return {
                content: [{ type: 'text', text: formatTaskDetail(task, runtime, callbackBinding) }],
                details: {
                  action: 'get',
                  profile,
                  taskId,
            ...
            case 'delete': {
              ...
              invalidateAppTopics('tasks');
              return {
                content: [{ type: 'text', text: `Deleted scheduled task @${taskId}.` }],
                details: {
                  action: 'delete',
                  profile,
                  taskId,
            ...
            case 'run': {
              ...
              return {
                content: [{ type: 'text', text: `Started scheduled task @${taskId} as run ${result.runId}.` }],
                details: {
                  action: 'run',
                  profile,
                  taskId,
