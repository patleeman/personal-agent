...
If a reusable skill needs external tools, prefer including `mcp.json` beside `SKILL.md` with the same `mcpServers` format used by `mcp_servers.json`. This is the canonical shape for skill-owned MCPs because the workflow docs and server config travel together. Bundled skill MCP servers are merged into runtime MCP configuration, and explicit user config wins on server-name conflicts. See [MCP](../extensions/system-mcp/README.md).
