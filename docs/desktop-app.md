...
The embedded browser tracks a revision counter. When the user changes the page after the agent's last `browser_snapshot`, the next prompt injects `browser-changed-since-snapshot` context so the agent knows its old page observation is stale and should snapshot again.
