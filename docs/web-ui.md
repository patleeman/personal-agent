...
- live conversations now include an `image` tool that can generate a single inline image through configured `openai-codex` or `openai` GPT-5 auth, including edit/variation flows from recent attached images or previously generated conversation images
